...
const ProjectsNavContainer = styled.div(({ theme, isDarkTheme }) => css`
  ...
  grid-column-gap: 2px;
  border-top: 1px solid ${isDarkTheme ? theme.separatorLight : theme.separator};
...
const Separator = styled.div(
  ({ theme, isDarkTheme }) => css`
    width: 1px;
    justify-self: center;
    background: ${isDarkTheme ? theme.separatorLight : theme.separator};
`)
  ...
  return (
    <ProjectsNavContainer isDarkTheme={isDarkTheme}>
      <PrevLink to={`/${baseUrl}/${prevProject.slug}`}>
        ...
        <Text>
          <Category>{prevProject.category[lang]}</Category>
          <ProjectNavTitle className='ProjectNavTitle'>{prevProject.label[lang]}</ProjectNavTitle>
        </Text>
      </PrevLink>
      <Separator isDarkTheme={isDarkTheme} />
      <NextLink to={`/${baseUrl}/${nextProject.slug}`}>
        <Text>
          <Category>{nextProject.category[lang]}</Category>
          <ProjectNavTitle className='ProjectNavTitle'>{nextProject.label[lang]}</ProjectNavTitle>
        </Text>
