...
import { LinkNav } from "components/_ui/Links";
...
const FooterInnerContainer = styled.div(({ theme, isDarkTheme }) => css`
  ...
  color: ${isDarkTheme ? theme.white : theme.secondaryColor};
  ${mq("small")} {
    text-align: center;
	...
	return (
		<FooterContainer isDarkTheme={isDarkTheme}>
			<FooterInnerContainer>
				...
				<Sitemap>
					<TitleList>Sitemap</TitleList>
					<List>
						{map(links, (link, key) => (
							<li key={key}>
								<LinkNav
									nav={!link.IS_EXTERNAL}
									href={link.IS_EXTERNAL ? link.SLUG_INDEX : undefined}
									to={link.IS_EXTERNAL ? undefined : `/${link.SLUG_INDEX}`}
				...
				<FollowUs>
					<TitleList>FormattedMessage: footer-followus</TitleList>
					<List>
				...
				<Mentions>
					...
					<LinkTerms to={`/${ROUTES.TERMS.SLUG_INDEX}`}>
						{ROUTES.TERMS.LABEL}
					</LinkTerms>
				</Mentions>
			</FooterInnerContainer>
