...
import { headerDistance } from "components/_ui/PageHeader";
...
const OuterContainerNavMinified = styled.div(
	({ isModalOpen, isLimitReached }) => css`
  label: "outer-container-nav-minified";
  position: fixed;
  z-index: 3;
  top: 0;
  left: 0;
...
const Header = withTheme(({ isHeaderDark, isBodyDark, isHome }) => {
	...
	return (
		<>
			...
			<NavMinified isLimitReached={isLimitReached} isModalOpen={isModalOpen}>
				<ButtonModal onClick={toggleModal} isModalOpen={isModalOpen}>
					<OpenMenuStyled
						isModalOpen={isModalOpen}
