...
const ButtonInitial = styled(
  ({ style, className, children }) => <animated.div style={style} className={className} children={children} />
)(({ alignLeft }) => css`
  position: absolute;
  left: ${alignLeft ? 0 : '50%'};
...
const ButtonMargin = styled(ButtonLabel)(({ theme }) => css`
  margin-left: 1px;
  margin-right: 1px;
  white-space: nowrap;
  ${mq('small')} {
...
const ButtonContainer = styled(animated.div)(({ theme }) => css`
  ${mq('small')} {
    flex-grow: 1;
    width: 100%;
  }
  ...
  return (
    <Container alignLeft={alignLeft}>
      ...
      <ButtonContainer style={stylesButtonLeft}>
        <CopyToClipboard text={emailValue} onCopy={showFeedback}>
          <ButtonMargin large isOpen={isOpen}>
            <FormattedMessage
              id={isCopied ? 'contact-button-copied' : 'contact-button-copy'}
            />
          </ButtonMargin>
        </CopyToClipboard>
      </ButtonContainer>
      ...
      <ButtonContainer style={stylesButtonRight}>
        <ButtonMargin large right href={`mailto:${emailValue}`} target='_blank' isOpen={isOpen}>
