...
export default function PicsContainer(props) {
  ...
  useEffect(() => {
    ...
    const setDisplacementTarget = (value) => {
      ...
      if (displacementTarget.current + value > 0) {
        ...
        displacementTarget.current =
          -(props.itemsLength * props.size - nodeWidth) - 1;
      } else {
        displacementTarget.current += value;
      }
    };
    ...
    const onDrag = (e) => {
      if (dragging.current) {
        if (e.changedTouches) {
          setDisplacementTarget(
            (e.changedTouches[0].clientX - touchCurrent) * 2
          );
  ...
  useEffect(() => {
    ...
    const update = () => {
      const newD =
        displacementCurrent.current +
        (displacementTarget.current - displacementCurrent.current) * 0.15;
  ...
  return (
    ...
    >
      <Slide x={displacement}>{props.children}</Slide>
    </Container>
  );
}
